...
void ANNbd_shrink::ann_search(ANNdist box_dist, ANNSearchData& a)
{
	...
	for (int i = 0; i < n_bnds; i++) {			// is query point in the box?
		if (bnds[i].out(a.ANNkdQ)) {				// outside this bounding side?
												// add to inner distance
			inner_dist = (ANNdist) ANN_SUM(inner_dist, bnds[i].dist(a.ANNkdQ));
		}
	}
	if (inner_dist <= box_dist) {				// if inner box is closer
		child[ANN_IN]->ann_search(inner_dist, a);	// search inner child first
		child[ANN_OUT]->ann_search(box_dist, a);	// ...then outer child
	}
	else {										// if outer box is closer
		child[ANN_OUT]->ann_search(box_dist, a);	// search outer child first
		child[ANN_IN]->ann_search(inner_dist, a);	// ...then outer child
	}
